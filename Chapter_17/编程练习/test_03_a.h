...
//操作:在链表的末尾添加项
//前提条件:item是一个待添加至链表的值,plist指向一个已初始化的链表
//后置条件:如果可以,该函数在链表末尾添加一个项,且返回true,否则返回false
bool AddItem(Item item,List * plist);

//操作:把函数作用于链表的每一项 plist指向一个已初始化的链表 pfun指向一个函数,该函数接受一个Item类型的参数,且无返回值
//后置条件:pfun指向的函数作用于链表中的每一项一次
void Traverse(const List * plist,void(*pfun)(Item item));

// 操作:释放已分配的内存(如果有的话) plist指向一个已初始化的链表
// 后置条件:释放了为链表分配的所有内存,链表为空
